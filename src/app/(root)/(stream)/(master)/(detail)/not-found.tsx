...
import { StreamDetailNotFoundView } from "./not-found.view";

async function getTitle({}: NotFoundMetadataUtilityInput = {}) {
  return msg({ message: "Not Found • poppy" });
}
...
export default async function StreamDetailNotFound({}: NotFoundInput) {
  await connection();

  return (
    <>
      <Metadata title={await getTitle()} />
      <StreamDetailNotFoundView />
    </>
  );
}
