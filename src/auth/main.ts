...
export const authConfig = {
  ...
  cookies: {
    callbackUrl: {
      ...
      options: {
        domain: process.env.POPPY__COOKIES__DOMAIN || undefined,
      },
    },
    csrfToken: {
  ...
  debug: (process.env.POPPY__DEBUG || "true") === "true",
  pages: {
    error: "/auth/error",
    newUser: "/auth",
    signIn: "/auth/login",
  ...
  secret: splitSecrets(process.env.POPPY__SECRETS__AUTH || "secret"),
  session: {
